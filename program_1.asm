...
.code   ; Keep procedures here
    encode PROC
        ...
        cond1: ; if(( plain >= 'a' && plain <= 'x') || (plain => 'A' && plain <= 'X'))
            ...
            comp_ge plain, 'a'
            comp_le plain, 'x'
            branch_and cond1_true ; Checks the and bit of the TF register. If set to 1, go to the endif_L label
            move 0, TF  ; reset the TF register for the second comparison pair
 
            ; (plain => 'A' && plain <= 'X')
            comp_ge plain, 'A'
            comp_le plain, 'X'
            branch_and cond1_true
            move 0, TF  ; reset the TF register
            jump else_L
            ; Second 
            cond1_true:
                add plain, 2, GP4
